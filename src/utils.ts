...
export class Utils {
  ...
  static shouldSizeToContent(n: GridStackNode | undefined): boolean {
    return (
      n?.grid && (!!n.sizeToContent || (n.grid.opts.sizeToContent && n.sizeToContent !== false))
    );
  }
  ...
  static sort(nodes: GridStackNode[], dir: 1 | -1 = 1, column?: number): GridStackNode[] {
    ...
    if (dir === -1)
      return nodes.sort(
        (a, b) => (b.x ?? 1000) + (b.y ?? 1000) * column - ((a.x ?? 1000) + (a.y ?? 1000) * column),
      );
    else
      return nodes.sort(
        (b, a) => (b.x ?? 1000) + (b.y ?? 1000) * column - ((a.x ?? 1000) + (a.y ?? 1000) * column),
      );
  }
  ...
  static removeStylesheet(id: string): void {
    const el = document.querySelector('STYLE[gs-style-id=' + id + ']');
